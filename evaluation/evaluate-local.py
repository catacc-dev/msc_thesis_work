...
INPUT_DIRECTORY = Path("/home/catarina_caldeira/Desktop/input_TH")
OUTPUT_DIRECTORY = Path("/home/catarina_caldeira/Desktop/evaluation/output_th")
GROUND_TRUTH_DIRECTORY = Path("/home/catarina_caldeira/Desktop/ground_truth_th")
...
def main():
    ...
    init_pool(_image_evaluator, _segmentation_evaluator, _debug, has_predictions)

    for prediction in tqdm(predictions):
        metrics["results"].append(process(prediction))
    ...
    aggregate_functions = [
        ...
        {
            'name': '25pc',
            'function': partial(np.nanquantile, q=0.25)
        },
        {
            'name': '50pc',
            'function': partial(np.nanquantile, q=0.50)
        },
        {
            'name': '75pc',
            'function': partial(np.nanquantile, q=0.75)
        },
        {
            'name': 'count',
            'function': lambda arr: np.sum(~np.isnan(arr))
        },
...
def write_metrics(*, metrics):
    ...
    with open(OUTPUT_DIRECTORY / "metrics.json", "w") as f:
        f.write(json.dumps(metrics, cls=NpEncoder, indent=4))
